...
`REP` and conditional `REPcc` repeat one scalar instruction in program order. A faulting iteration does not decrement the counter; a successful iteration decrements it before an event can be admitted. An event between iterations restarts through the repeat prefix rather than restoring hidden continuation state.
...
The software contract includes a 64-bit ELF environment, an LP64 C ABI, and compiler-facing target operations. Optional instruction groups are discovered through `CPUID`, whose results define a compatibility boundary. The floating-point extension adds sixteen 64-bit floating-point registers and status state. The scalable-vector extension adds 32 vector registers and 16 packed predicate registers at an implementation-selected VLEN from 128 through 2048 bits, with dedicated discovery, saved-state, ABI, and DWARF contracts. Approximate transcendental operations use individually discoverable accuracy contracts with stable identifiers and certified error bounds.
