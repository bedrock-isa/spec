...
Every page-table entry is one naturally aligned 64-bit descriptor. A leaf owns
(:ref:base.memory.translation.PTE.PFN:)\texttt{[55:14]}, while a table pointer owns
(:ref:base.memory.translation.PTE.NEXT_TABLE:)\texttt{[55:12]}. Both layouts therefore have a 56-bit
architectural physical-address ceiling. An implementation reports its exact
(:ref:base.cpuid.IMPLEMENTATION.ADDRESS_WIDTHS.PARAMETERS.PABITS:) value, from 32 through 56, through CPUID
\texttt{ADDRESS\_WIDTHS}. In a present descriptor, bits of the layout-selected
physical-address field at or above that value (:term:base.terms.must_be_zero:).
...
(:pte-field-target:base.memory.translation.PTE.NEXT_TABLE:)\texttt{NEXT\_TABLE} & \texttt{55..12} & physical base address of the next table object\\
...
An L4-to-L3 or L3-to-L2 pointer targets a 16-KiB table object and is malformed
when (:ref:base.memory.translation.PTE.NEXT_TABLE:)\texttt{[13:12]} is nonzero. An L2-to-L1 pointer targets a
4-KiB table object and may use all four values of those bits. Every walker,
PTQUERY implementation, and page-walk cache preserves the complete
(:ref:base.memory.translation.PTE.NEXT_TABLE:) value while descending.
...
faults or is squashed. In that latter case, (:ref:base.memory.translation.PTE.A:) is an imprecise, PTE-owned usage
hint; it is not instruction progress, restart state, or generic continuation state.
(:ref:base.memory.translation.PTE.D:) is set only when the corresponding store or atomic update
