...
size is determined by the implementation-selected VLEN. Source-language
spellings are defined by the applicable compiler extension rather than this
calling convention.
\end{description}
...
When the scalable-vector extension is present, let $B$ be VLEN in bytes. A
...
carries no value. VLEN is stable until reset, so every object and frame in one
execution context uses one size.
...
size, or only a fixed-width prefix does not preserve the register.
...
Frame pointers are otherwise omitted.
...
59..63 & --- & reserved\\
64..95 & \texttt{V0}..\texttt{V31} & assigned when VECTOR is present\\
96..111 & \texttt{P0}..\texttt{P15} & assigned when VECTOR is present\\
...
complete VLEN-dependent image. Predicate register locations contain the
packed image without private spill padding.
...
A compound scalable value is INDIRECT. A scalable value that cannot use its
direct register class is also converted to INDIRECT: the caller creates an
object with the layout above and assigns its pointer through the GENERAL
procedure. No scalable value is split between registers or copied directly
into an ordinary fixed-size argument slot.
...
register class does not exhaust GENERAL.
...
contains the pointer, not the scalable object.
...
\input{isa/interfaces/abi/generated/c_return_register_quick_reference.tex}
...
\input{isa/interfaces/abi/generated/c_call_relocation_quick_reference.tex}
...
The Bedrock ELF ABI defines TLS models, symbols, and relocations. The active
platform/runtime environment ABI defines thread setup, the initial \texttt{GS0}
image, loader handoff, and OS-facing TLS transitions.
...
The following guarantees apply to ordinary normal-memory accesses. They do not by
themselves establish C inter-thread ordering:

\input{isa/interfaces/abi/generated/ordinary_memory_access_guarantees.tex}
...
\input{isa/interfaces/abi/generated/native_atomic_primitive_quick_reference.tex}
...
The examples illustrate the assignment procedure and do not introduce
exceptions to the normative rules above.
...
\manualabicase{scalable-register-classes}
...
\texttt{F0}. The direct vector result uses \texttt{V0}. No class advances a
cursor belonging to another class.
...
The assembly examples are non-normative. File directives, basic-block
comments, and local labels that do not affect the shown control flow are
omitted. The scalar leaf example presents its one-instruction counted loop with \texttt{REP}; the counter supplies
...
\end{tabularx}
\endgroup
\manualfigurecaption{Scalar leaf function}
\end{manualformblock}
...
\end{tabularx}
\endgroup
\manualfigurecaption{Non-leaf function with a tail transfer}
