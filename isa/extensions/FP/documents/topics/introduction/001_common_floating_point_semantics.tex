...
The \texttt{S} and \texttt{D} suffixes select 32- and 64-bit floating-point scalar arithmetic sizes. H selects the
16-bit scalar conversion and storage format only. Conditional floating-point moves place the integer condition
suffix in the mnemonic.


The H, S, and D formats are IEEE-754 binary16, binary32, and binary64. H uses sign bit 15, exponent bits 14..10,
and fraction bits 9..0. S uses sign bit 31, exponent bits 30..23, and fraction bits
...
one is quiet. The architectural default quiet NaNs are \texttt{0x7e00} for H, \texttt{0x7fc00000} for S, and
...
An H conversion reads bits 15..0 of an H-valued Fn operand and an H result write writes zero to bits 63..16. An S
operation reads bits 31..0 of each Fn operand. Writing an S result to Fn writes the result to bits 31..0 and
...
(:ref:FP.registers.STATE.FSTATUS.RM:) before the operation. All conversion causes, including causes from NaN handling and narrowing,
