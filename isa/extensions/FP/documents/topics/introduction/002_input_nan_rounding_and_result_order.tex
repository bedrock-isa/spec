...
\item If (:ref:FP.registers.STATE.FSTATUS.DAZ:) is set, replace each subnormal input with a zero having the same sign.
...
quiet NaN. When (:ref:FP.registers.STATE.FSTATUS.DN:) is set, replace the selected NaN with the default quiet NaN.
\item Round a numeric result to the selected format using (:ref:FP.registers.STATE.FSTATUS.RM:), except where an instruction names a
...
\item If (:ref:FP.registers.STATE.FSTATUS.FTZ:) is set and the rounded result is a nonzero subnormal, replace it with a zero of the
