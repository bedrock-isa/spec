Let $E$ be the selected element width in bytes, $N=VLEN/(8E)$ using the
current VLEN, and $G$ be
...
predicate image below VLEN/8 are cleared, while bits at or above VLEN/8 are
preserved.

If \texttt{G AND NOT Ps} is empty, the instruction completes normally with
the low VLEN/8-bit portion of \texttt{Ps} equal to \texttt{G}. Otherwise it selects one pending lane $i$, calculates that
...
another lane is attempted. Each entry or re-entry re-evaluates the current
VLEN. A lane-completion boundary is not an instruction
