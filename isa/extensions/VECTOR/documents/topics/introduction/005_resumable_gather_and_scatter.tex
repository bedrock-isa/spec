...
For element width $E$ bytes, let $N=VLEN/(8E)$ using the current VLEN. Define $G=typed_E(Pg)$ by
copying \texttt{Pg[iE]} to bit $iE$ for every $0\leq i<N$ and clearing every
other predicate bit below VLEN/8. At each entry or re-entry, the
instruction re-evaluates VLEN and performs the
...
transaction faults. Consequently inactive completion bits below the current
VLEN/8 are zero, while bits at or above VLEN/8 retain their prior values. If
\texttt{pending} is empty, the
instruction
retires without a memory transaction and its normal postcondition is
low VLEN/8-bit portion of \texttt{Ps} equals \texttt{G}.
...
\begin{BedrockTabular}{@{}>{\raggedright\arraybackslash}p{1.50in}>{\raggedright\arraybackslash}p{0.75in}>{\raggedright\arraybackslash}p{3.15in}@{}}
\toprule
\rowcolor{BedrockHeaderFill}
\textbf{Canonical address} & \textbf{Constraint} & \textbf{Lane offset} \\
\midrule
...
lane.  Any later report is an imprecise
(:ref:base.events.EXCEPTION.MACHINE_CHECK:) delivered at a subsequent lane or
retirement boundary after the store becomes irrevocable.  These instructions are not permitted for MMIO, including when only
