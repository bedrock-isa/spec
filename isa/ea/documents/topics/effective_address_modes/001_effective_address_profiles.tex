...
the compact comparison table identifies the profile-specific encodings. An effective-address (:term:base.terms.field:) is an operand
...
descriptor bytes are appended as one operand\textquotesingle{}s (:term:base.terms.payload:) sequence. Sequences appear in declared instruction operand
order. For two payload-bearing operands, the encoded stream contains the (:term:base.terms.opcode_space:) for both operands, the complete
operand 0 (:term:base.terms.payload:) sequence, the complete operand 1 (:term:base.terms.payload:) sequence, and any trailing (:term:base.terms.padding:), in that order.

Multi-byte (:term:base.terms.payload|plural:), such as displacements, absolute addresses, and immediates, are little-endian. Payload
names ending in s are signed and are sign-extended before use; (:term:base.terms.payload:) names without s are unsigned unless the
