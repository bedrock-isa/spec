...
\item Execution-stage integer faults and enabled floating-point exception conditions that raise
\texttt{FLOATING\_POINT\_FAULT}.
\end{enumerate}
...
reads the return stack before validating its target; and Long Call, Long Return, Event Return, and supervisor-entry
...
\texttt{old\_sp}-8 &
one source value or the following instruction\textquotesingle{}s continuation PC at \texttt{old\_sp}-8\\
\texttt{POP}, \texttt{RET} &
$[\texttt{old\_sp},\texttt{old\_sp}+8)$ &
\texttt{old\_sp}+8 &
one destination value or continuation PC read at \texttt{old\_sp}\\
\texttt{PUSHP}, \texttt{FPUSHP} &
$[\texttt{old\_sp}-16,\texttt{old\_sp})$ &
\texttt{old\_sp}-16 &
listed pair\textquotesingle{}s second register at \texttt{old\_sp}-16 and first register at \texttt{old\_sp}-8\\
\texttt{POPP}, \texttt{FPOPP} &
$[\texttt{old\_sp},\texttt{old\_sp}+16)$ &
\texttt{old\_sp}+16 &
listed pair\textquotesingle{}s second register from \texttt{old\_sp}, then first register from \texttt{old\_sp}+8\\
\texttt{LCALL} &
$[\texttt{old\_sp}-16,\texttt{old\_sp})$ &
\texttt{old\_sp}-16 &
continuation PC at \texttt{old\_sp}-16 and return CS image at \texttt{old\_sp}-8\\
\texttt{LRET} &
$[\texttt{old\_sp},\texttt{old\_sp}+16)$ &
\texttt{old\_sp}+16 &
continuation PC at \texttt{old\_sp} and return CS image at \texttt{old\_sp}+8\\
\bottomrule
\end{manualtabular}
...
SYSCALL is the SYSTEM\_CALL event; user-origin entry keeps its continuation in the user context bank described by
...
Auto-update EA forms update temporary operand-evaluation state. The architectural register update becomes
...
writes do not exist. FFLAGS remains an IEEE-754 accrued exception-cause bank, so FFLAGS fields that an instruction does
