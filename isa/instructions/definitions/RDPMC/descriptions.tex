...
modulo $2^{64}$. Cold reset and warm RESET clear them to zero. (:ref:base.control_registers.PMC.EN:) controls whether they increment.

\Needspace{1.25in}
\BedrockTableCaption{Standard Performance Counters}
...
\midrule
\texttt{1} & \texttt{CYCLE} & each implementation cycle while (:ref:base.control_registers.PMC.EN:) is set\\
...
common discovery (:term:base.terms.header:). Other IDs beyond the standard set are implementation-defined and are discovered through that
leaf.

\begin{BedrockRaggedBlock}
Every privilege level may read every implemented counter. Clearing (:ref:base.control_registers.PMC.EN:) freezes the current counter values
without clearing them.
\end{BedrockRaggedBlock}
