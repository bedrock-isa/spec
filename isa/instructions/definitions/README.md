...
The exact contract is documented in `SCHEMA.md`. The dataclass decoder and
`schema.lock` reject unknown fields, wrong scalar types, invalid discriminated
variants, and changes to the decoder contract.

Each instruction directory contains two required YAML documents:

```text
operation.yaml    typed operation contract, public metadata, artifacts, and semantic cases
...
Operation records carry privilege, repeat eligibility, flags, events, and other
typed cross-artifact facts so reference tables and compact effect lines are
generated from one source. Handwritten explanatory text remains authored prose, but descriptions
...
`cpuid_flags.yaml` independently owns each CPUID flag's stable ID, public
token, and architectural location. An extension manifest lists only its local
`required_cpuid_flags`; the loader conjoins those with requirements inherited
from its parent extensions. Operation cases may add registered flags for their
typed form/selector/profile partition, and Decode IR preserves that exact
resolved relation for execution and generated metadata.
...
wiring and its local CPUID flag requirement. The external numerical provider owns
