...
Each instruction names its required \texttt{FPTRANSA\_ACCURACY} contract. A clear \texttt{PRESENT} field for that
contract raises \texttt{ILLEGAL\_INSTRUCTION} before the instruction reads operands or floating-point state.
...
A signaling NaN generates NV. NaN results are quieted and then processed by \texttt{FSTATUS.DN}.
\texttt{FSTATUS.FTZ} replaces a subnormal result with the required signed zero and generates UF. Each instruction
generates NV and DZ according to its special-value rules, and generates OF and UF when required by its exact result;
NX is never generated, remains unchanged in FFLAGS, and cannot cause the \texttt{FLOATING\_POINT\_FAULT}
...
instruction raises the \texttt{FLOATING\_POINT\_FAULT} architectural event before writing any destination. Otherwise
