...
presents its one-instruction counted loop with \texttt{REP}. For a grouped body
that does not write the selected loop-control register, REPG's post-body rule
requires an equivalent form to bypass an initial zero but permits it to supply
N directly for a positive N; the displayed \texttt{REP} also takes N directly
and annuls zero. Apart from that zero-case normalization, instruction order and
operands are unchanged. If an implementation sequence conflicts with a
