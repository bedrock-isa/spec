...
memory-model lowering. It depends on the
...
\item[object pointer] A 64-bit value that designates an object or one-past an object according to the C abstract machine. In the canonical interoperable context, the value is a pre-segment virtual address.
\item[function pointer] A 64-bit value naming a C entry point. In the canonical interoperable context, the value is a pre-segment code address.
...
An ordinary object or function pointer is 64 bits wide and 8-byte aligned. In
the canonical interoperable context its value is a canonical pre-segment
virtual address. Object and function pointers remain distinct C type categories
...
are caller-saved scratch segment registers; no incoming value in them is
preserved across a C call.

\subsection{C Address-Space Contexts}
The canonical interoperable C context uses disabled or bounds-only
\texttt{CS}, \texttt{DS}, and \texttt{SS}. In this context, ordinary object and
function pointers are canonical pre-segment virtual addresses and conforming
execution maintains all of the following invariants:

\begin{itemize}
\item \texttt{CS}, \texttt{DS}, and \texttt{SS} are disabled with a zero
mantissa or are enabled in bounds-only mode.
...
\item Every C object and its one-past address must be representable as a
nonzero 64-bit address; no object may have mathematical one-past address
$2^{64}$.
\end{itemize}

The active platform ABI may instead execute C code with translated
\texttt{CS}, \texttt{DS}, or \texttt{SS} images. That contract owns the
pointer and symbol coordinate systems; compatibility among code, ordinary
data, stack-derived addresses, and TLS-derived addresses; context transitions;
and adaptation at every external boundary. In particular, independently
translated \texttt{DS} and \texttt{SS} contexts do not by themselves make a
stack-derived pointer usable as an ordinary data pointer. Code using such a
context is not interoperable with the canonical context unless the active
platform contract defines the necessary compatibility and transitions.

The 64-bit all-zero null-pointer representation remains unchanged in every
context, and address zero is not assigned to a C object or function. Whether a
null-derived access is guaranteed to fault is a mapping or guard-page policy of
the active platform rather than a property of the representation.
...
\texttt{PC} is at CFA minus 8 and \texttt{CS} has a same-value rule. Signal
...
\item If an argument cannot be placed completely in the remaining
...
The general and floating-point resources exhaust independently.
...
A C function pointer is 64 bits wide. In the canonical interoperable context,
its value is a pre-segment near-call address. Code and data pointers have the
same size and alignment, but remain distinct C type categories; neither
representation creates an interchangeable code/data address space.
