...
author displayed cells and connections explicitly. ``predicate-width`` authors
...
import yaml
from latex_builder.common import tex_escape


GENERATOR_KIND = "vector-example"
VISIBLE_BYTES = 16
...
def _decode_lane_map(raw: dict[str, Any], path: Path, scalable: bool) -> VectorExample:
    ...
    if len(result_rows) != 1:
        _fail(path, "lane-map requires exactly one destination-after row")
    ...
    return VectorExample("lane-map", scalable, tuple(rows), edges)
...
def _decode_width_map(raw: dict[str, Any], path: Path, scalable: bool) -> VectorExample:
    if not scalable:
        _fail(path, "width-map requires scalable continuation")
    if "container_bits" in raw:
        return _decode_detailed_width_map(raw, path, scalable)
    _check_keys(raw, path, "width-map root", {"kind", "view", "rows", "edges"})
    example = _decode_lane_map({**raw, "kind": "lane-map"}, path, scalable)
    if any(len(row["cells"]) != 4 for row in example.rows):
        _fail(path, "width-map pilot requires four explicit 32-bit containers")
    return VectorExample("width-map", scalable, example.rows, example.edges)
...
def _decode_detailed_width_edges(
    ...
    for index, item_value in enumerate(_list(value, path, "edges")):
        ...
        if effect not in WIDTH_CONNECTION_EFFECTS:
            _fail(path, f"edges[{index}].effect is not a width-map connection")
        if display not in WIDTH_CONNECTION_DISPLAYS:
            _fail(path, f"edges[{index}].display is not a width-map presentation")
...
def _decode_detailed_width_map(
    ...
    for index, value in enumerate(_list(raw["rows"], path, "rows")):
        ...
        if role == "predicate":
            if row["element_bits"] != 8:
                _fail(path, "detailed width-map predicate row uses byte positions")
            cells = tuple(
                _cell(cell, path, f"rows[{index}].cells[{cell_index}]")
                for cell_index, cell in enumerate(
                    _list(row["cells"], path, f"rows[{index}].cells")
            ...
            if len(cells) != VISIBLE_BYTES:
                _fail(path, "detailed width-map predicate row must cover 16 bytes")
            if any(cell["effect"] not in {"set", "clear", "ignored"} for cell in cells):
                _fail(path, "detailed width-map predicate cells must be set, clear, or ignored")
            decoded = {
                "id": row_id,
                "label": label,
                "role": role,
            ...
            if role not in {"source", "destination-after"}:
                _fail(path, f"rows[{index}].role is invalid for a detailed width-map")
            containers = tuple(
    ...
    if sorted(roles) != ["destination-after", "predicate", "source"]:
        _fail(path, "detailed width-map requires one source, result, and predicate row")
    ...
    if not source_effects <= {"copy", "ignored"}:
        _fail(path, "detailed width-map source cells must be copied or discarded")
    result_effects = {
        cell["effect"]
    ...
    if not result_effects <= {"copy", "preserve", "sign-fill", "zero"}:
        _fail(path, "detailed width-map result cells have an invalid classification")
    rows_by_id = {row["id"]: row for row in rows}
    edges = _decode_detailed_width_edges(raw["edges"], path, rows_by_id, result_row)
    return VectorExample("width-map", scalable, tuple(rows), edges)


def _decode_stateful_predicate_range(
    raw: dict[str, Any], path: Path, scalable: bool
) -> VectorExample:
    _check_keys(raw, path, "predicate-range root", {"kind", "view", "states", "range", "result"})
    if not scalable:
        _fail(path, "stateful predicate-range requires scalable continuation")
    ...
    if len(states) != 2 or {state["id"] for state in states} != {"remaining", "offset"}:
        _fail(path, "stateful predicate-range requires remaining and offset states")
    ...
    if result["element_bits"] != 16:
        _fail(path, "stateful predicate-range uses complete W predicate groups")
    groups = tuple(
        _decode_width_container(
            group,
            path,
    ...
    if len(groups) != 8:
        _fail(path, "stateful predicate-range must cover all eight visible W groups")
...
def _decode_predicate_range(raw: dict[str, Any], path: Path, scalable: bool) -> VectorExample:
    ...
    if not scalable:
        _fail(path, "predicate-range requires scalable continuation")
    ...
    if result["element_bits"] != 16:
        _fail(path, "predicate-range pilot uses explicit W positions")
    cells = tuple(_cell(cell, path, f"result.cells[{index}]") for index, cell in enumerate(_list(result["cells"], path, "result.cells")))
    if len(cells) != 8:
        _fail(path, "predicate-range result must cover all eight visible W positions")
    if any(cell["effect"] not in PREDICATE_EFFECTS for cell in cells):
        _fail(path, "predicate-range cells must explicitly set or clear every position")
    return VectorExample(
        "predicate-range",
        scalable,
        (
...
def _decode_predicate_width(
    ...
    if scalable:
        _fail(path, "predicate-width requires a fixed 16-byte example view")
    _check_keys(
        raw,
        path,
        "predicate-width root",
    ...
    if (source_bits, result_bits) not in {(16, 8), (8, 16)}:
        _fail(path, "predicate-width supports only the reviewed 16-to-8 and 8-to-16 examples")
    if result["write"] != "complete":
        _fail(path, "predicate-width result.write must be complete")
    ...
    if source_bits == 16:
        if source_start_term != 0 or destination_start_term not in {0, "source-lanes"}:
            _fail(path, "16-to-8 predicate-width maps all source lanes to one destination half")
        if count_term != "source-lanes":
            _fail(path, "16-to-8 predicate-width count must be source-lanes")
    else:
        if source_start_term not in {0, "destination-lanes"} or destination_start_term != 0:
            _fail(path, "8-to-16 predicate-width maps one source half to all destination lanes")
        if count_term != "destination-lanes":
            _fail(path, "8-to-16 predicate-width count must be destination-lanes")
    if source_start + count > source_lanes or destination_start + count > destination_lanes:
        _fail(path, "predicate-width mapping exceeds the fixed example lane bounds")
    ...
    return VectorExample("predicate-width", False, rows, edges)


def _decode_predicate_lane_map(
    raw: dict[str, Any], path: Path, scalable: bool
) -> VectorExample:
    _check_keys(raw, path, "predicate-lane-map root", {"kind", "view", "rows", "edges"})
    raw_rows = _list(raw["rows"], path, "rows")
    if len(raw_rows) not in {2, 3}:
        _fail(path, "predicate-lane-map requires two or three authored rows")
    rows: list[dict[str, Any]] = []
    row_ids: set[str] = set()
    for index, value in enumerate(raw_rows):
        ...
        if role not in {"source", "destination-before", "destination-after"}:
            _fail(path, f"{where}.role is invalid for a predicate-lane-map")
    ...
    if len(result_rows) != 1 or result_rows[0]["storage"] != "predicate":
        _fail(path, "predicate-lane-map requires one predicate destination-after row")
    ...
    if transfer_targets != copied_targets:
        _fail(path, "predicate-lane-map transfers must cover exactly the copied result cells")
    return VectorExample("predicate-lane-map", scalable, tuple(rows), tuple(edges))
...
def _decode_scalar_bridge(
    ...
    if not scalable:
        _fail(path, "scalar-bridge requires scalable continuation")
    _check_keys(raw, path, "scalar-bridge root", {"kind", "view", "rows", "scalars", "connections"})
    raw_rows = _list(raw["rows"], path, "rows")
    if len(raw_rows) != 1:
        _fail(path, "scalar-bridge requires exactly one authored vector row")
    ...
    for index, value in enumerate(_list(raw["scalars"], path, "scalars")):
        ...
        if role not in {"source", "index", "destination"}:
            _fail(path, f"{where}.role is invalid for scalar-bridge")
        scalars.append(
            {
                "id": scalar_id,
    ...
    if len({scalar["role"] for scalar in scalars}) != len(scalars):
        _fail(path, "scalar-bridge scalar roles must be unique")
    ...
    if "index" in scalars_by_role:
        if len(controls) != 1:
            _fail(path, "scalar-bridge indexed forms require exactly one index control connection")
        control = controls[0]
        ...
        if controls:
            _fail(path, "scalar-bridge unindexed broadcast cannot contain control connections")
        controlled_lane = None

    if row["role"] == "source":
        if set(scalars_by_role) != {"index", "destination"}:
            _fail(path, "scalar-bridge extraction requires one index and one scalar destination")
        if len(selected_cells) != 1:
            _fail(path, "scalar-bridge extraction requires exactly one selected source cell")
        ...
        if any(
            ...
            _fail(path, "scalar-bridge extraction source cells must display one selected source lane")
        if len(transfers) != 1:
            _fail(path, "scalar-bridge extraction requires exactly one displayed transfer")
        ...
        if set(scalars_by_role) != {"source", "index"}:
            _fail(path, "scalar-bridge insertion requires one scalar source and one index")
        if len(selected_cells) != 1:
            _fail(path, "scalar-bridge insertion requires exactly one selected result cell")
        selected_lane = next(iter(selected_cells))
        if any(
            (cell["effect"], cell.get("appearance"))
            != (
            ...
            _fail(path, "scalar-bridge insertion must preserve every unselected result lane")
        if len(transfers) != 1:
            _fail(path, "scalar-bridge insertion requires exactly one displayed transfer")
        transfer = transfers[0]
        transfer_index = connections.index(transfer)
        if (
        ...
        if set(scalars_by_role) != {"source"}:
            _fail(path, "scalar-bridge broadcast requires exactly one scalar source")
        if selected_cells:
            _fail(path, "scalar-bridge broadcast cannot contain a selected result lane")
        if any(
            (cell["effect"], cell.get("appearance")) != ("copy", "source")
            for cell in row["cells"]
        ):
            _fail(path, "scalar-bridge broadcast result cells must all display the scalar source")
        expected_transfers = {
        ...
        if len(transfers) != len(expected_transfers) or actual_transfers != expected_transfers:
            _fail(path, "scalar-bridge broadcast transfers must cover every result lane exactly once")
    return VectorExample(
        "scalar-bridge",
        True,
        (row,),
...
def _decode_memory_lanes(
    ...
    if not scalable:
        _fail(path, "memory-lanes requires scalable continuation")
    _check_keys(
        raw,
        path,
        "memory-lanes root",
    ...
    if element_bits not in {8, 16, 32, 64}:
        _fail(path, "memory-lanes element_bits must be 8, 16, 32, or 64")
    lane_count = VISIBLE_BYTES * 8 // element_bits
    ...
    if predicate["element_bits"] != 8:
        _fail(path, "memory-lanes predicate uses byte positions")
    ...
    if len(predicate_cells) != VISIBLE_BYTES:
        _fail(path, "memory-lanes predicate must cover all 16 byte positions")
    ...
    if controls != active or transfers != active:
        _fail(path, "memory-lanes connections must cover exactly every active access")
    if any((index in active) != (result_cells[index]["effect"] == "copy") for index in range(lane_count)):
        _fail(path, "memory-lanes result classifications must match active accesses")
    return VectorExample("memory-lanes", True, rows, tuple(connections), {"base": base_data})
...
def _decode_conversion_map(
    ...
    if not scalable:
        _fail(path, "conversion-map requires scalable continuation")
    _check_keys(raw, path, "conversion-map root", {"kind", "view", "container_bits", "source", "result", "predicate", "connections"})
    container_bits = _integer(raw["container_bits"], path, "container_bits")
    if container_bits not in {32, 64}:
        _fail(path, "conversion-map container_bits must be 32 or 64")
    container_count = VISIBLE_BYTES * 8 // container_bits
    rows: list[dict[str, Any]] = []
    for section_name, role in (("source", "source"), ("result", "destination-after")):
        ...
        if section_name == "result" and element_bits == rows[0]["element_bits"]:
            _fail(path, "conversion-map source and result widths must differ")
        containers = tuple(
            _decode_width_container(
                container,
                path,
    ...
    _check_keys(predicate, path, "predicate", {"label", "element_bits", "cells"})
    if predicate["element_bits"] != 8:
        _fail(path, "conversion-map predicate uses byte positions")
    predicate_cells = tuple(
        _cell(cell, path, f"predicate.cells[{index}]")
        for index, cell in enumerate(_list(predicate["cells"], path, "predicate.cells"))
    )
    if len(predicate_cells) != VISIBLE_BYTES:
        _fail(path, "conversion-map predicate must cover all 16 byte positions")
    active_containers = {
        container_index
    ...
    if copied_results != edge_targets:
        _fail(path, "conversion-map connections must cover exactly the converted result fields")
    return VectorExample("conversion-map", True, tuple(rows), edges)


def load(path: Path) -> VectorExample:
    raw = _mapping(yaml.safe_load(path.read_text(encoding="utf-8")), path, "root")
...
def _render_scalar_bridge_tikz(example: VectorExample) -> str:
    ...
    scalar_by_id = {scalar["id"]: scalar for scalar in scalars}
...
def render_tikz(example: VectorExample) -> str:
    lines: list[str] = []
    if example.variant == "scalar-bridge":
        return _render_scalar_bridge_tikz(example)
    if example.variant == "memory-lanes":
        return _render_memory_lanes_tikz(example)
    if example.variant == "reduction":
        return _render_reduction_tikz(example)
    if example.variant == "conversion-map":
    ...
    if example.variant == "predicate-range":
        ...
        return "\n".join(lines)
    if example.variant == "width-map" and any(
