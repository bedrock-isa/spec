...
from site_visuals import extract_visuals, render_visuals
...
def _instruction_index(
    ...
    for set_name, title, _introduction, instructions in gen_docs.instruction_set_groups(
        ...
        group_label = gen_docs.instruction_set_page_label(set_name)
        lines.extend(
            [
        ...
        for instruction in instructions:
            lines.append(
                ...
                + _link(
                    ...
                    scoped_target(
                        document.id,
                        gen_docs.instruction_label(instruction.mnemonic),
                    ),
...
def render_site_output(
    ...
    *,
    ...
    site = build_site(document_specs, model)
    written: set[str] = set()
    ...
    for document in document_specs:
        ...
        if document.id == "isa":
            _write_page(
                ...
                _instruction_index(site, document, model),
                written,
            )
