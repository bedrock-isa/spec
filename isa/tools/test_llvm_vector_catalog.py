...
class LLVMVectorCatalogTests(unittest.TestCase):
    def test_normalized_form_count_and_generated_id_set(self) -> None:
        ...
        self.assertEqual(len(forms), 243)
        self.assertEqual(len(ids), 243)
        self.assertEqual(
            Counter(form["class"] for form in forms),
            Counter({"long": 49, "extralong": 91, "xxlong": 103}),
        )

        # The closed allocation has 238 source rows. Its one fieldless VDUP
        ...
        self.assertEqual(len(set(source_ids)), 238)
    ...
    def test_repeat_table_is_derived_and_excludes_vector(self) -> None:
        entries = gen_llvm_vector_catalog._load_repeat_eligibility()
        self.assertIn(("add", False, True, True), entries)
        self.assertFalse(any(mnemonic.startswith("v") for mnemonic, *_ in entries))
