...
def register_model_figure(spec: dict[str, Any]) -> str:
    ...
    def emit_group(start_row: int, names: list[str], group_label: str) -> None:
        label_tex = r"\\".join(tex_escape(part) for part in group_label.split("\\"))
        y_top = -start_row * 0.235
        rows.append(rf"\node[anchor=south] at (0,{y_top + 0.02:.3f}) {{{width - 1}}};")
        rows.append(rf"\node[anchor=south] at (2.075,{y_top + 0.02:.3f}) {{{width // 2 - 1}}};")
        rows.append(rf"\node[anchor=south] at (4.15,{y_top + 0.02:.3f}) {{0}};")
        for offset, name in enumerate(names):
            y = -(start_row + offset) * 0.235 - 0.18
            rows.append(rf"\draw (0,{y:.3f}) rectangle (4.15,{y + 0.18:.3f});")
            rows.append(rf"\draw (2.075,{y:.3f}) -- (2.075,{y + 0.18:.3f});")
            rows.append(rf"\node[anchor=west] at (4.25,{y + 0.09:.3f}) {{{tex_escape(name)}}};")
        ...
        rows.append(rf"\node[anchor=west] at ({brace_x + 0.18:.3f},{(bracket_top + y_bottom) / 2:.3f}) {{\shortstack[l]{{{label_tex}}}}};")

    row = 0
    emit_group(row, [f"D{i}" for i in range(d_count)], "DATA\\REGISTERS")
    row += d_count + 1
    emit_group(row, [f"A{i}" for i in range(a_count)], "ADDRESS\\REGISTERS")
    row += a_count + 1
    emit_group(row, ["SP"], "STACK\\POINTER")
    row += 2
    emit_group(row, ["PC"], "PROGRAM\\COUNTER")
    row += 2
    emit_group(row, ["FLAGS", "STATUS"], "CONTROL\\STATE")
    rows.extend(
        [
            r"\end{tikzpicture}",
...
def supervisor_stack_frame(control: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "layout": [
            ...
            {"offset": 0x08, "name": "RESERVED", "description": "reserved, must be zero"},
