...
appear on the same instruction as REPG, which forms a one-instruction group. The
group is illegal if ENDG is not found before the grouping window ends, if ENDG
appears without an active REPG group, or if another repeat prefix is nested
inside the group.
...
not force a particular implementation strategy. Instead, it asks the assembler
to reject the group unless the source satisfies the REPG-fast eligibility rules
that can be checked statically. The assembler checks the grouped instruction
classes, rejects nested repeat forms, rejects control or state-changing
operations, and still enforces the grouping-window containment rule for both
\texttt{REPG} and \texttt{REPGF}. \texttt{REPGF} additionally rejects writes to
the selected counter register and rejects PC-relative addressing forms. Runtime
properties such as ordinary
mapped memory remain architectural preconditions of the fast class rather than
extra encoded bits.
...
operations are not eligible for the base REPG form. These are base legality
constraints, so an assembler rejects them for both \texttt{REPG} and
\texttt{REPGF}.
