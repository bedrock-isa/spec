...
same as executing the repeated instruction or byte-counted instruction group in
program order under the selected counter and termination rules. For REPcc, the
instruction body of the terminating condition-false iteration commits, while
...
metadata and repeated-execution accumulation rules.
\end{itemize}
...
\texttt{REPG Dn, imm16} is a compact instruction immediately followed by the
grouped instruction body. The immediate is an unsigned byte count, not a word
count. It must be nonzero and even, and the byte-counted body end must land on
an instruction boundary. Instructions inside the body keep their own word 0
length fields and are decoded as ordinary instruction records under the active
REPG group.

REPG group discovery is bounded by the explicit byte count rather than by an
instruction-cache-line window. Because every instruction record carries an
explicit length, instruction interpretation can walk from the first body
instruction through exactly \texttt{imm16} body bytes without interpreting the
full semantics of each grouped instruction.
...
selected counter register, the REPG body length in bytes, and a byte
displacement from the saved PC back to the first grouped instruction:
\[
\mathit{group\_start\_pc} =
\mathit{fault\_pc} - \mathit{body\_start\_delta\_bytes}.
\]
...
\item use REPG for a byte-counted group of counted operations;
