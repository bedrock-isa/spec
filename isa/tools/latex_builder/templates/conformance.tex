...
Canonical assembly text and exact conformance encodings are a source-to-byte and byte-to-text contract for architecture tools.
They do not change the architectural result of an instruction record.
...
as a summary, example, guide, or validation status.
...
\item Summary tables, programming guides, and validation-status files provide navigation or verification context
...
\texttt{isa/reference/conformance\_manifest.yaml} names each required vector family, its source, and the cases that
must remain represented. The manifest covers assembler encodings, atomic ordering, paging and cross-page accesses,
stack and event boundaries, memory-order obligations, cache synchronization, and floating-point special values.
Every required case is the ID of a concrete case in the named YAML source.

For an input named by a vector, the expected bytes, state transition, architectural event, or allowed and forbidden
outcome is normative for that case. General behavior continues to follow the defining architecture rule. Schema
decoders, vector evaluators, formal models, and repository tests are validation tools; their implementation does
not add an architectural result.
