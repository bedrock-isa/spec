\clearpage
\section{Terminology and Compatibility}
...
A \emph{far-segment channel} is an architectural segment register loaded from a far data address so an
explicit-segment memory operation can perform the access. The optional C far-data profile uses GS2 through GS5 for
incoming argument channels and GS1 as the scratch and return channel.
...
DS, SS, and GS0 through GS5 from the SREG selector table, not CS, FLAGS, STATUS, PC, or SP.
CS may appear as the fixed source operand of \texttt{RDSEG CS} and \texttt{PUSH CS}; those forms do not add CS to
the SREG selector namespace and provide no corresponding write or pop form.
