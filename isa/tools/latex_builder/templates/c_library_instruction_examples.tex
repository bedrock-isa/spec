...
A negative counter can be paired with a one-past-the-end base address to traverse memory
in increasing address order while the counter moves toward zero. A positive counter can
be paired with a one-before-the-beginning base address to traverse memory in decreasing
address order.
...
\subsection{memcpy}
The forward byte-copy form uses one-past-the-end source and destination bases and a
negative signed count.

\par\begingroup\small\ttfamily
\begin{tabularx}{\linewidth}{@{}X@{}}
; memcpy(dst, src, n), byte copy forward\tabularnewline
; A0 = src + n, A1 = dst + n\tabularnewline
; D0 = -n\tabularnewline
REP D0, MOV.B [A0 + D0 * 1], [A1 + D0 * 1]\tabularnewline
\end{tabularx}
\endgroup\par
...
destination overlaps at a higher address, the copy runs backward using positive index
values.

\par\begingroup\small\ttfamily
\begin{tabularx}{\linewidth}{@{}X@{}}
; memmove(dst, src, n), overlapping backward byte copy\tabularnewline
; A0 = src - 1, A1 = dst - 1\tabularnewline
; D0 = +n\tabularnewline
REP D0, MOV.B [A0 + D0 * 1], [A1 + D0 * 1]\tabularnewline
\end{tabularx}
\endgroup\par
...
; A1 = dst + n, D1 = byte c\tabularnewline
; D0 = -n\tabularnewline
REP D0, MOV.B D1, [A1 + D0 * 1]\tabularnewline
\end{tabularx}
\endgroup\par
...
; A0 = a + n, A1 = b + n\tabularnewline
; D0 = -n\tabularnewline
REPEQ D0, CMP.B [A0 + D0 * 1], [A1 + D0 * 1]\tabularnewline
\end{tabularx}
\endgroup\par
