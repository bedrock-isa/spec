...
\texttt{PTE.CP=11} & reserved & Reserved cache-policy encoding.\\
\bottomrule
\end{longtable}
\endgroup
...
\begingroup\footnotesize
\begin{longtable}{@{}p{0.9in}p{0.75in}p{3.8in}@{}}
\toprule
\textbf{Selector} & \textbf{Code} & \textbf{Architectural Ordering Effect}\\
\midrule
\endhead
\texttt{RELAXED} & 0 & Atomicity only. No ordering is imposed on unrelated memory operations.\\
\texttt{ACQUIRE} & 1 & Later memory operations by the same hardware thread are ordered after the atomic operation.\\
\texttt{RELEASE} & 2 & Earlier memory operations by the same hardware thread are ordered before the atomic operation.\\
\texttt{ACQREL} & 3 & Applies both acquire and release ordering.\\
\texttt{SEQCST} & 4 & Applies acquire-release ordering and participates in the global sequentially consistent order for atomic operations.\\
reserved & 5--7 & Reserved selector encodings; using one is invalid for the atomic form.\\
\bottomrule
\end{longtable}
\endgroup
