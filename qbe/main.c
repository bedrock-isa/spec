...
extern Target T_bedrock;
extern int bedrock_set_cmodel(char *);
...
main(int ac, char *av[])
{
	...
	FILE *inf, *hf;
	char *f, *sep;
	int c, asm, asm_explicit;

	asm = Defasm;
	asm_explicit = 0;
	T = Deftgt;
	outf = stdout;
	while ((c = getopt(ac, av, "hd:m:o:G:t:")) != -1)
		switch (c) {
		case 'd':
			for (; *optarg; optarg++)
				if (isalpha(*optarg)) {
		...
		case 't':
			for (tm=tmap;; tm++) {
				...
				if (strcmp(optarg, tm->name) == 0) {
					T = *tm->T;
					if (!asm_explicit && strcmp(optarg, "bedrock") == 0)
						asm = Gaself;
					break;
				}
			}
		...
		default:
			...
			fprintf(hf, "\t%-11s select Bedrock code model: small, low, high, large\n", "-m <model>");
