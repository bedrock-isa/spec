...
has unspecified contents. VLEN is stable until reset, so every object and frame in one
...
\texttt{signed int}. Other base types are target extensions and cross an
external ABI boundary under a separately documented compatible extension ABI.
...
The baseline ABI uses the natural aggregate layout above. An extension may
define packed aggregates, \texttt{\#pragma pack}, empty structures, or
zero-length arrays, and types using those facilities cross external ABI
boundaries under a separately documented compatible extension ABI.
...
\BedrockGeneratedCReturnRegisterTable
...
composition preserves the INDIRECT classification. An extension aggregate with
packed or unaligned members crosses this external ABI boundary under a
...
An explicit extension owns stronger placement requirements.
...
\BedrockGeneratedCAtomicPrimitiveTable

\Needspace{2.6in}
\subsection{C Atomic Memory Order Mapping}
\BedrockGeneratedCMemoryOrderTable
...
\BedrockGeneratedCFetchRmwTable
