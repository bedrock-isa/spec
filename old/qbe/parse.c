...
parserefl(int arg)
{
	...
	while (peek() != Trparen && peek() != Tdots) {
		...
		if (k == 4)
			if (arg)
				*curi = (Ins){Oargc, R, {TYPE(ty), r}, Kl, 0};
			else
				*curi = (Ins){Oparc, r, {TYPE(ty)}, Kl, 0};
		else if (env)
			if (arg)
				*curi = (Ins){Oarge, R, {r}, k, ptr};
			else
				*curi = (Ins){Opare, r, {R}, k, ptr};
		else
			if (arg)
				*curi = (Ins){Oarg, R, {r}, k, ptr};
			else
				*curi = (Ins){Opar, r, {R}, k, ptr};
		curi++;
		hasenv |= env;
		if (peek() == Trparen)
			break;
...
DoOp:
	...
	if (op == Talloc1 || op == Talloc2)
		op = Oalloc;
	if (k == 4)
		err("size class must be w, l, p, s, or d");
	if (op >= NPubOp)
	...
	for (b=fn->start; b; b=b->link) {
		...
		for (i=b->ins; i-b->ins < b->nins; i++)
			if (rtype(i->to) == RTmp) {
				...
				if (!first && t->isptr != i->ptr && t->cls == Kl)
					err("temporary %%%s is assigned with"
						" mixed pointer and integer types", t->name);
				t->isptr = i->ptr;
			}
	}
	for (b=fn->start; b; b=b->link) {
...
parsefn(int export)
{
	...
	if (peek() != Tglo) {
		int retptr = 0;
		rcls = parsecls(&curf->retty, &retptr);
		curf->retptr = retptr;
	} else
		rcls = 5;
	if (next() != Tglo)
