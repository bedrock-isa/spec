...
optyp(unsigned ctyp)
{
	return KIND(ctyp) == PTR ? 'l' : irtyp(ctyp);
}
...
psymb(Symb s)
{
	switch (s.t) {
	case Tmp:
		fprintf(of, "%%t%lld", s.u.n);
		break;
	case Var:
		fprintf(of, "%%%s", s.u.v);
		break;
	case Glo:
		fprintf(of, "$glo%lld", s.u.n);
		break;
	case Con:
		fprintf(of, "%lld", s.u.n);
		break;
	}
}
...
load(Symb d, Symb s)
{
	...
	psymb(d);
	t = irtyp(d.ctyp);
	fprintf(of, " =%c load%c ", t, memtyp(d.ctyp));
	psymb(s);
	fprintf(of, "\n");
}
...
expr(Node *n)
{
	...
	case 'N':
		sr.t = Con;
		sr.u.n = n->u.n;
		sr.ctyp = (n->u.n < -2147483648LL || n->u.n > 2147483647LL) ? LNG : INT;
		break;
	...
	case '=':
		...
		fprintf(of, "\tstore%c ", memtyp(s1.ctyp));
		goto Args;
	...
	if (n->op == 'P' || n->op == 'M') {
		fprintf(of, "\tstore%c ", memtyp(sl.ctyp));
		psymb(sr);
		fprintf(of, ", ");
		psymb(sl);
		fprintf(of, "\n");
...
Case *
mkcase(int is_default, long long value, Stmt *stmt, Case *next)
{
	Case *c;
...
Case *
caseappend(Case *list, Case *item)
{
	Case *c;
...
static int
pow2minus1(long long n)
{
	if (n < 0)
		return 0;
	return ((unsigned long long)n & ((unsigned long long)n + 1u)) == 0;
}

static int
switch_mask(Node *n, long long *mask)
{
	if (!n || n->op != '&')
		return 0;
	if (numlit(n->r, mask) || numlit(n->l, mask))
		return pow2minus1(*mask);
	return 0;
}
...
nodeeq(Node *a, Node *b)
{
	...
	case 'N':
		return a->u.n == b->u.n;
	case 'V':
		return strcmp(a->u.v, b->u.v) == 0;
	case 0:
		return 0;
	default:
		return nodeeq(a->l, b->l) && nodeeq(a->r, b->r);
	}
}
...
static int
split_add_const(Node *n, Node **base, long long *constant)
{
	if (!n)
		return 0;
	if (numlit(n, constant)) {
		*base = 0;
		return 1;
	}
	if (n->op == '+') {
		if (numlit(n->r, constant)) {
			*base = n->l;
			return *base == 0 || intvar(*base);
		}
		if (numlit(n->l, constant)) {
			*base = n->r;
			return *base == 0 || intvar(*base);
		}
	}
...
static int
add_switch_table(long long *values, long long nvalue)
{
	char *text, *p;
	long long i;
	size_t cap;
	...
	cap = 8 + (size_t)nvalue * 24u;
	text = alloc(cap);
	p = text;
	p += sprintf(p, "{ w");
	for (i=0; i<nvalue; i++)
		p += sprintf(p, " %lld", values[i]);
	sprintf(p, " }");
	glo = nglo;
	ini[nglo++] = text;
	return glo;
...
static int
try_switch_table(Node *exprn, Case *cases)
{
	Case *c;
	Node *base, *common_base, *rexpr;
	Symb idx, off, tab, ptr, val, bval, ret;
	long long mask, nvalue, cnst, defval, i;
	long long *values;
	char ity;
	int *seen;
	int glo;

	if (!switch_mask(exprn, &mask) || mask < 0 || mask > 255)
		return 0;
	nvalue = mask + 1;
	common_base = 0;
	defval = 0;
	for (c=cases; c; c=c->next)
		if (c->is_default) {
			rexpr = ret_expr(c->stmt);
			if (!split_add_const(rexpr, &common_base, &defval))
				return 0;
			break;
		}
	if (!c)
		return 0;
	if (common_base && !intvar(common_base))
		return 0;
	values = alloc((size_t)nvalue * sizeof values[0]);
	seen = alloc((size_t)nvalue * sizeof seen[0]);
	for (i=0; i<nvalue; i++) {
		values[i] = defval;
		seen[i] = 0;
	}
	for (c=cases; c; c=c->next) {
		...
		if (!rexpr)
			return 0;
		if (!split_add_const(rexpr, &base, &cnst))
			return 0;
		if (!nodeeq(base, common_base))
			return 0;
		if (c->is_default)
			continue;
		if (c->value < 0 || c->value >= nvalue || seen[c->value])
			return 0;
		values[c->value] = cnst;
		seen[c->value] = 1;
	}

	glo = add_switch_table(values, nvalue);
	idx = expr(exprn);
	if (irtyp(idx.ctyp) != 'l')
		sext(&idx);
	off.t = Tmp;
...
int
switchstmt(Node *exprn, Case *cases, int b)
{
	Case *c, *n;
	Symb x;
	char ty;
	int end, def, next, body, falls, have_next, all_return;

	if (try_switch_table(exprn, cases))
		return 1;
	...
	for (c=cases; c; c=c->next) {
		...
		body = lbl++;
		next = have_next ? lbl++ : def;
		fprintf(of, "\t%%t%d =w ceq%c ", tmp, ty);
		psymb(x);
		fprintf(of, ", %lld\n", c->value);
...
%token <n> STR
%token <n> IDENT
%token PP MM LE GE SHL SHR SIZEOF

%token TVOID TINT TLNG
%token IF ELSE SWITCH CASE DEFAULT WHILE FOR BREAK RETURN
...
prot: IDENT '(' par0 ')'
{
	...
	for (t=0, n=$3; n; t++, n=n->r) {
		s = varget(n->u.v);
		m = SIZE(s->ctyp);
		fprintf(of, "\t%%%s =p alloc%d %d\n", n->u.v, m, m);
		fprintf(of, "\tstore%c %%t%d", memtyp(s->ctyp), t);
		fprintf(of, ", %%%s\n", n->u.v);
	}
};
...
cases: cases caseitem                { $$ = caseappend($1, $2); }
     |                               { $$ = 0; }
     ;

caseitem: CASE NUM ':' RETURN expr ';'
                                     { $$ = mkcase(0, $2->u.n, mkstmt(Ret, $5, 0, 0), 0); }
     | DEFAULT ':' RETURN expr ';'   { $$ = mkcase(1, 0, mkstmt(Ret, $4, 0, 0), 0); }
     ;
