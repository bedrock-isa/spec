	...
	for (b=fn->start; b; b=b->link) {
		...
		for (i=b->ins; i-b->ins < b->nins; i++) {
			if (!req(i->to, R)) {
				...
				if (i->op == Oaddr)
					ptr = 1;
				if (i->op == Ocopy && rtype(i->arg[0]) == RTmp)
					ptr |= tmp[i->arg[0].val].isptr;
				if (i->op == Oadd && i->cls == Kl) {
					if (rtype(i->arg[0]) == RTmp)
						ptr |= tmp[i->arg[0].val].isptr;
					if (rtype(i->arg[1]) == RTmp)
						ptr |= tmp[i->arg[1].val].isptr;
				}
				tmp[t].width = w;
				tmp[t].ndef++;
				tmp[t].cls = i->cls;
...
refindex(int t, Fn *fn)
{
	...
	r = newtmp(fn->tmp[t].name, fn->tmp[t].cls, fn);
	fn->tmp[r.val].isptr = fn->tmp[t].isptr;
	return r;
}
