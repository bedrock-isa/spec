...
class DocumentationBuildTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        for group in sorted(build_docs.OWNER_MODULES):
            for source_path in sorted((build_docs.ROOT / "sail" / "model" / group).glob("*.sail")):
    ...
    def test_semantic_index_counts_routes_and_stability(self) -> None:
        ...
        self.assertEqual(route_only, {"BNDUXX", "FETCHXOR", "LEA"})
        self.assertEqual(
    ...
    def test_mocked_build_is_confined_and_repeatable(self) -> None:
        sail_root = build_docs.ROOT / "sail"
        before = {
            path.relative_to(sail_root): path.read_bytes()
            for path in sail_root.rglob("*")
            if path.is_file()
        }
        ...
        def fake_sail(command, *, cwd, check):
            ...
            output = Path(cwd) / "sail_doc" / "bedrock-sail.json"
        ...
        self.assertFalse(any(path.name == "__pycache__" for path in sail_root.rglob("*")))
